...
public class DataInitService {
    ...
    public void initialiserDonneesMassives(int nbAuteurs, int nbLivres, int nbCategories) {
        EntityManager em = emf.createEntityManager();
        em.getTransaction().begin();
        ...
        for (int i = 1; i <= nbAuteurs; i++) {
            Auteur auteur = new Auteur();
            auteur.setNom("Auteur " + i);
            auteur.setPrenom("Prenom " + i);
            em.persist(auteur);

            for (int j = 1; j <= nbLivres; j++) {
                Livre livre = new Livre();
                livre.setTitre("Livre " + i + "-" + j);
                livre.setAuteur(auteur);
                livre.getCategories().add(categories.get(rand.nextInt(nbCategories)));
                em.persist(livre);
            }
        }
        ...
        System.out.println("Données massives insérées : " +
                nbAuteurs + " auteurs, " + (nbAuteurs * nbLivres) + " livres, " + nbCategories + " catégories");
    }
}
